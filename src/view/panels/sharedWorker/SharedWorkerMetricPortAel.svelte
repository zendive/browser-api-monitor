<script lang="ts">
  ...
  let { metrics }: { metrics: ISharedWorkerPortAelMetric[] } = $props();
  const { sortSharedWorkerPortAel } = useConfigState();
  const sortedMetrics = $derived.by(() =>
    metrics.toSorted(compareByFieldOrder(
      sortSharedWorkerPortAel.field,
      sortSharedWorkerPortAel.order,
    ))
  );
  let isExpanded = $state(true);

  function updateSort(
    field: keyof ISharedWorkerPortAelMetric,
    order: ESortOrder,
  ) {
    sortSharedWorkerPortAel.field = field;
    sortSharedWorkerPortAel.order = order;
    saveLocalStorage({ sortSharedWorkerPortAel });
  }
</script>
    ...
    <thead class="sticky-header">
      <tr>
        <th class="w-full">
          ...
          />
          <ColumnSortable
            sort={sortSharedWorkerPortAel}
            by="firstSeen"
            update={updateSort}
          >
            port.addEventListener [<Variable value={sortedMetrics.length} />]
          </ColumnSortable>
        </th>
        <th class="ta-c">
          <ColumnSortable
